...
app.get("/pets", (req, res) => {
  ...
  app.get("/pets/:id", (req, res) => {
    ...
    client.query(`SELECT name, age, kind FROM pets WHERE id = $1`, [petId])
      .then((data) => {
  ...
  app.post("/pets", (req, res) => {
      ...
      client.query(`INSERT INTO pets (name, kind, age) VALUES ($1, $2, $3) RETURNING *`, [name, kind, age])
        .then((data) => {
  ...
  app.patch("/pets/:id", (req, res) => {
      const petId = Number.parseInt(req.params.id);
      const { kind, name } = req.body;
      let age = Number.parseInt(req.body.age) || null;
      if(!kind && !name && !age) {
        res.sendStatus(422);
        return;
      }
  ...
  app.delete("/pets/:id", (req, res) => {
    ...
    client.query("DELETE FROM pets WHERE id = $1 RETURNING *", [id])
    .then((data) => {
      if (data.rows.length === 0) {
        res.sendStatus(404);
      } else {
